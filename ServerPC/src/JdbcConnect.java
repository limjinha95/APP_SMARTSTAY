...
public class JdbcConnect {
	...
	public String MyCoupon(String ID) throws SQLException {
		String query = "select coupon.name, coupon.startdate, coupon.enddate, coupon.info, coupon.cost from usecoupon join coupon on usecoupon.couponidx = coupon.idx where userid = ?";
		psmt = con.prepareStatement(query);
		psmt.setString(1, ID);
		rs = psmt.executeQuery();
		if (rs.next() == false) {
